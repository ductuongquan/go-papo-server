...
var searchFlags = [...]string{"from", "page", "in", "before", "after", "on"}
...
func ParseSearchParams(text string, timeZoneOffset int) []*SearchParams {
	...
	for _, flagPair := range flags {
		...
		if flag == "in" || flag == "page" {
			inPages = append(inPages, value)
		} else if flag == "from" {
	...
	if len(plainTerms) == 0 && len(hashtagTerms) == 0 && (len(inPages) != 0 || len(fromUsers) != 0 || len(afterDate) != 0 || len(beforeDate) != 0 || len(onDate) != 0) {
		paramsList = append(paramsList, &SearchParams{
			Terms:          "",
			IsHashtag:      false,
			InPages:     inPages,
