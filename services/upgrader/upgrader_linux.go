...
func verifySignature(filename string, sigfilename string, publicKey string) error {
	...
	if err != nil {
		mlog.Error("Unable to open the Ladifire .tar file to verify the file signature", mlog.Err(err))
		return NewInvalidSignature()
	}

	signature, err := os.Open(sigfilename)
	if err != nil {
		mlog.Error("Unable to open the Ladifire .sig file verify the file signature", mlog.Err(err))
		return NewInvalidSignature()
	}

	_, err = openpgp.CheckDetachedSignature(keyring, mattermost_tar, signature)
	if err != nil {
		mlog.Error("Unable to verify the Ladifire file signature", mlog.Err(err))
		return NewInvalidSignature()
	}
	return nil
}
...
func CanIUpgradeToE0() error {
	...
	if model.BuildEnterpriseReady == "true" {
		...
		return errors.New("you cannot upgrade your server from TE to E0 because you are already running Ladifire Enterprise Edition")
	}
	return nil
}
...
func UpgradeToE0() error {
	...
	if err != nil {
		...
		mlog.Error("Unable to get the path of the Ladifire executable", mlog.Err(err))
		return err
	}
	...
	if err != nil {
		...
		upgradeError = fmt.Errorf("error downloading the new Ladifire server binary file (percentage: %d)", upgradePercentage)
		mlog.Error("Unable to download the Ladifire server binary file", mlog.Int64("percentage", upgradePercentage), mlog.String("url", getCurrentVersionTgzUrl()), mlog.Err(err))
		upgradePercentage = 0
		return err
	}
	...
	if err != nil {
		...
		mlog.Error("Unable to download the signature file of the new Ladifire server", mlog.String("url", getCurrentVersionTgzUrl()+".sig"), mlog.Err(err))
		upgradePercentage = 0
		return err
	}
...
func extractBinary(executablePath string, filename string) error {
	...
	for {
		...
		if err == io.EOF {
			return errors.New("unable to find the Ladifire binary in the downloaded version")
		}
