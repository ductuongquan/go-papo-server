...
//		table.ColMap("AuthData").SetMaxSize(128).SetUnique(true)
//		table.ColMap("AuthService").SetMaxSize(32)
//		table.ColMap("FacebookToken").SetMaxSize(256) // chính xác là 255, nhưng chưa chắc chắn
//		table.ColMap("Email").SetMaxSize(128).SetUnique(true)
//		table.ColMap("Nickname").SetMaxSize(64)
//		table.ColMap("FirstName").SetMaxSize(64)
//		table.ColMap("LastName").SetMaxSize(64)
...
//}
//
//func (us SqlUserStore) UpdateLocale(userId string, newLocale string) store.StoreChannel {
//	return store.Do(func(result *store.StoreResult) {
//
//		if _, err := us.GetMaster().Exec("UPDATE Users SET Locale = :Locale WHERE Id = :UserId", map[string]interface{}{"Locale": newLocale, "UserId": userId}); err != nil {
//			result.Err = model.NewAppError("SqlUserStore.UpdateLocale", "store.sql_user.update_locale.app_error", nil, "user_id="+userId, http.StatusInternalServerError)
//		} else {
//			result.Data = newLocale
//		}
//	})
//}
//
...
//
//	searchQuery := `
//		SELECT Users.id, Users.AuthData, Users.FirstName, Users.LastName, Users.Locale , TeamMembers.roles as roles, TeamMembers.SchemeAdmin, TeamMembers.DisplayName FROM Users, TeamMembers
...
//
//	return store.Do(func(result *store.StoreResult) {
//		var users []*model.TeamMemberProfile
//
//		if _, err := us.GetReplica().Select(&users, searchQuery, parameters); err != nil {
...
//}
//
//func (us SqlUserStore) GetById(id string) store.StoreChannel {
//	return store.Do(func(result *store.StoreResult) {
//
//		user := model.User{}
//
//		if err := us.GetReplica().SelectOne(&user, "SELECT * FROM Users WHERE Id = :Id", map[string]interface{}{"Id": id}); err != nil {
//			result.Err = model.NewAppError("SqlUserStore.GetByEmail", store.MISSING_ACCOUNT_ERROR, nil, "id="+id+", "+err.Error(), http.StatusInternalServerError)
//		}
//
//		result.Data = &user
//	})
