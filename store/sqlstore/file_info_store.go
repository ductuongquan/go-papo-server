...
func (fs SqlFileInfoStore) InvalidateFileInfosForPageCache(pageId string) {
	fileInfoCache.Remove(pageId)
	if fs.metrics != nil {
...
func (fs SqlFileInfoStore) GetForPage(pageId string, readFromMaster bool, allowFromCache bool, offset, limit int) ([]*model.FileInfo, error) {
	if allowFromCache {
		if cacheItem, ok := fileInfoCache.Get(pageId); ok {
			if fs.metrics != nil {
	...
	if _, err := dbmap.Select(&infos,
		`SELECT
			...
			`, map[string]interface{}{"PageId": pageId, "Limit": limit, "Offset": offset}); err != nil {
		return nil, model.NewAppError("SqlFileInfoStore.GetForPost",
			"store.sql_file_info.get_for_post.app_error", nil, "post_id="+pageId+", "+err.Error(), http.StatusInternalServerError)
	} else {
		if len(infos) > 0 {
			fileInfoCache.AddWithExpiresInSecs(pageId, infos, FILE_INFO_CACHE_SEC)
		}
...
func (fs SqlFileInfoStore) AttachToMessage(fileId, messageId, creatorId string) (*model.FileInfo, error) {
	sqlResult, err := fs.GetMaster().Exec(
		`UPDATE
				...
				WHERE
					Id = :Id
					AND MessageId = ''
					AND CreatorId = :CreatorId`, map[string]interface{}{"MessageId": messageId, "Id": fileId, "CreatorId": creatorId})
	if err != nil {
		return nil, model.NewAppError("SqlFileInfoStore.AttachToPost",
			"store.sql_file_info.attach_to_post.app_error", nil, "post_id="+messageId+", file_id="+fileId+", err="+err.Error(), http.StatusInternalServerError)
	}

	count, err := sqlResult.RowsAffected()
	if err != nil {
		// RowsAffected should never fail with the MySQL or Postgres drivers
		return nil, model.NewAppError("SqlFileInfoStore.AttachToMessage",
			"store.sql_file_info.attach_to_post.app_error", nil, "post_id="+messageId+", file_id="+fileId+", err="+err.Error(), http.StatusInternalServerError)
	} else if count == 0 {
		// Could not attach the file to the post
		return nil, model.NewAppError("SqlFileInfoStore.AttachToMessage",
			"store.sql_file_info.attach_to_post.app_error", nil, "post_id="+messageId+", file_id="+fileId, http.StatusBadRequest)
	}
