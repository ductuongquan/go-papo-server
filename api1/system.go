// Copyright (c) 2018-present Ladifire. All Rights Reserved.
...
func updateConfig(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	cfg.PluginSettings.EnableUploads = c.App.GetSanitizedConfig().PluginSettings.EnableUploads
...
func getAnalytics(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	rows, err := c.App.GetAnalytics(name, pageId)
	if err != nil {
		c.Err = err
		return
	}
...
func getRedirectLocation(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		// hacker attempting to use Ladifire to probe a private network.
		w.Write([]byte(model.MapToJson(m)))
		return
	}
