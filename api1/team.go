...
func (api *API) InitTeam() {
	...
	api.BaseRoutes.TeamMember.Handle("/validate", api.ApiSessionRequired(validateTeamMemberToAdd)).Methods("GET")
	...
	api.BaseRoutes.TeamMember.Handle("/patch", api.ApiSessionRequired(patchTeamMember)).Methods("PUT")
...
func validateTeamMemberToAdd(c *Context, w http.ResponseWriter, r *http.Request) {
	c.RequireTeamId().RequireUserId()
	if c.Err != nil {
		return
	}
	...
	if err := c.App.ValidateUserBeforeAddToTeam(c.Params.TeamId, c.Params.UserId); err != nil {
		c.Err = err
		return
	}
...
func addTeamMember(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if err := c.App.ValidateUserBeforeAddToTeam(member.TeamId, member.UserId); err != nil {
		c.Err = err
		return
	}

	// TODO: prevent user add a team member with full permissions, e.g: if user is not has full permissions in team
	// then this user want to add a member with full permissions to team.

	member, err = c.App.AddTeamMember(member.TeamId, member.UserId, member.Roles, member.DisplayName)
	...
	if user, err2 = c.App.GetUser(member.UserId); err2 != nil {
		c.Err = err2
		return
	}
	...
	var teamMemberProfile = model.TeamMemberProfile{
		Id: user.Id,
	...
	w.WriteHeader(http.StatusCreated)
	w.Write([]byte(model.TeamMemberProfileToJson(&teamMemberProfile)))
}
...
func patchTeamMember(c *Context, w http.ResponseWriter, r *http.Request) {
	c.RequireTeamId().RequireUserId()
	if c.Err != nil {
		return
	}
	...
	if _, err := c.App.UpdateTeamMember(c.Params.TeamId, c.Params.UserId, newRoles, displayName); err != nil {
		c.Err = err
		return
	}
