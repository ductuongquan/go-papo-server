...
func uploadFile(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if err := r.ParseMultipartForm(*c.App.Config().FileSettings.MaxFileSize); err != nil && err != http.ErrNotMultipart {
		...
		if !c.App.SessionHasPermissionToChannel(c.App.Session, pageId, model.PERMISSION_UPLOAD_FILE) {
		...
		props := m.Value
		if len(props["page_id"]) == 0 {
			c.SetInvalidParam("page_id")
			return
		}
		pageId := props["page_id"][0]

		fmt.Println(m.File["files"])
...
func getFile(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if info.CreatorId != c.App.Session.UserId && !c.App.SessionHasPermissionToChannelByPost(c.App.Session, info.PageId, model.PERMISSION_READ_CHANNEL) {
