...
func (c *Context) LogAudit(extraInfo string) {
	audit := &model.Audit{UserId: c.App.Session.UserId, IpAddress: c.IpAddress, Action: c.Path, ExtraInfo: extraInfo, SessionId: c.App.Session.Id}
...
func (c *Context) LogAuditWithUserId(userId, extraInfo string) {
	...
	audit := &model.Audit{UserId: userId, IpAddress: c.IpAddress, Action: c.Path, ExtraInfo: extraInfo, SessionId: c.App.Session.Id}
	if r := <-c.App.Srv.Store.Audit().Save(audit); r.Err != nil {
		c.LogError(r.Err)
	}
}
...
func (c *Context) RequirePostId() *Context {
	...
	// post_id có dạng 156165165_156516516516
	if len(c.Params.PostId) < 0 {
...
func (c *Context) RequireConversationId() *Context {
	if c.Err != nil {
		return c
	}

	if len(c.Params.ConversationId) != 26 {
...
//func (c *Context) RequireFanpageId() *Context {
//	if c.Err != nil {
//		return c
//	}
//
