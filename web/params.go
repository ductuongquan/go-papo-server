...
func ParamsFromRequest(r *http.Request) *Params {
	...
	if val, ok := props["page_id"]; ok {
		params.PageId = val
	}

	if val, ok := props["page_ids"]; ok {
		params.PageIds = val
	}
	...
	if val, ok := props["comment_id"]; ok {
		params.CommentId = val
	}
	...
	if val, ok := props["conversation_id"]; ok {
		params.ConversationId = val
	}

	if val, ok := props["snippet_id"]; ok {
		params.SnippetId = val
	}

	if val, ok := props["tag_id"]; ok {
		params.TagId = val
	}

	if val, ok := props["note_id"]; ok {
		params.NoteId = val
	}
