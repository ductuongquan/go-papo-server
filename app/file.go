...
import (
	...
	"bitbucket.org/enesyteam/papo-server/services/filesstore"
...
func (a *App) getInfoForFilename(page *model.Fanpage, teamId, channelId, userId, oldId, filename string) *model.FileInfo {
...
//)
//
//func (app *App) FileBackend() (filesstore.FileBackend, *model.AppError) {
//	license := app.License()
//	return filesstore.NewFileBackend(&app.Config().FileSettings, license != nil && *license.Features.Compliance)
//}
//
//func (app *App) ReadFile(path string) ([]byte, *model.AppError) {
//	backend, err := app.FileBackend()
//	if err != nil {
//		return nil, err
//	}
//	return backend.ReadFile(path)
...
//
//// Caller must close the first return value
//func (app *App) FileReader(path string) (io.ReadCloser, *model.AppError) {
//	backend, err := app.FileBackend()
//	if err != nil {
//		return nil, err
//	}
...
//}
//
//func (app *App) FileExists(path string) (bool, *model.AppError) {
//	backend, err := app.FileBackend()
//	if err != nil {
//		return false, err
//	}
...
//}
//
//func (app *App) MoveFile(oldPath, newPath string) *model.AppError {
//	backend, err := app.FileBackend()
//	if err != nil {
//		return err
//	}
...
//}
//
//func (app *App) WriteFile(fr io.Reader, path string) (int64, *model.AppError) {
//	backend, err := app.FileBackend()
//	if err != nil {
//		return 0, err
//	}
...
//}
//
//func (app *App) RemoveFile(path string) *model.AppError {
//	backend, err := app.FileBackend()
//	if err != nil {
//		return err
//	}
...
//}
//
//func (app *App) GetInfoForFilename(page *model.Fanpage, teamId string, filename string) *model.FileInfo {
//	// Find the path from the Filename of the form /{pageId}/{userId}/{uid}/{nameWithExtension}
//	split := strings.SplitN(filename, "/", 5)
//	if len(split) < 5 {
//		mlog.Error("Unable to decipher filename when migrating page to use FileInfos", mlog.String("page_id", page.Id), mlog.String("filename", filename))
//		return nil
//	}
//
//	pageId := split[1]
...
//
//	if split[0] != "" || split[1] != page.PageId || strings.Contains(split[4], "/") {
//		mlog.Warn(
//			"Found an unusual filename when migrating post to use FileInfos",
//			mlog.String("page_id", page.Id),
//			mlog.String("channel_id", page.PageId),
//			mlog.String("filename", filename),
//		)
//	}
//
//	pathPrefix := fmt.Sprintf("teams/%s/pages/%s/users/%s/%s/", teamId, pageId, userId, oldId)
//	path := pathPrefix + name
//
//	// Open the file and populate the fields of the FileInfo
...
//	info.CreateAt = page.CreateAt
//	info.UpdateAt = page.UpdateAt
//	info.Path = path
//
//	if info.IsImage() {
//		nameWithoutExtension := name[:strings.LastIndex(name, ".")]
...
//
//// Creates and stores FileInfos for a post created before the FileInfos table existed.
//func (app *App) MigrateFilenamesToFileInfos(page *model.Fanpage) []*model.FileInfo {
//	if len(page.Filenames) == 0 {
//		mlog.Warn("Unable to migrate page to use FileInfos with an empty Filenames field", mlog.String("page_id", page.PageId))
//		return []*model.FileInfo{}
//	}
//
...
//	//
//	//// There's a weird bug that rarely happens where a post ends up with duplicate Filenames so remove those
//	filenames := utils.RemoveDuplicatesFromStringArray(page.Filenames)
//	//
//	//result := <-cchan
//	//if result.Err != nil {
//	//	mlog.Error(
...
//			mlog.String("page_id", page.PageId),
//		)
//	} else {
//		for _, filename := range filenames {
//			info := app.GetInfoForFilename(page, teamId, filename)
//			if info == nil {
//				continue
//			}
//
...
//	defer fileMigrationLock.Unlock()
//
//	//result = <-a.Srv.Store.Fanpage().Get(page.Id)
//	//if result.Err != nil {
...
//			mlog.Error(
//				fmt.Sprintf("Unable to save file info when migrating page to use FileInfos, err=%v", result.Err),
//				mlog.String("page_id", page.PageId),
//				mlog.String("file_info_id", info.Id),
//				mlog.String("file_info_path", info.Path),
//			)
...
//
//	// Copy and save the updated post
//	newPage := &model.Fanpage{}
//	*newPage = *page
//
//	newPage.Filenames = []string{}
//	newPage.FileIds = fileIds
//
//	// Update Pages to clear Filenames and set FileIds
//	if result := <-app.Srv.Store.Fanpage().Update(newPage, page); result.Err != nil {
//		mlog.Error(fmt.Sprintf("Unable to save migrated post when migrating to use FileInfos, new_file_ids=%v, old_filenames=%v, err=%v", newPage.FileIds, page.Filenames, result.Err), mlog.String("page_id", page.PageId))
//		return []*model.FileInfo{}
//	}
//	return savedInfos
//}
//
//func (app *App) GeneratePublicLink(siteURL string, info *model.FileInfo) string {
//	hash := GeneratePublicLinkHash(info.Id, *app.Config().FileSettings.PublicLinkSalt)
//	return fmt.Sprintf("%s/files/%v/public?h=%s", siteURL, info.Id, hash)
//}
//
//func GeneratePublicLinkHash(fileId, salt string) string {
...
//}
//
//func (app *App) UploadMultipartFiles(teamId string, pageId string, userId string, fileHeaders []*multipart.FileHeader, clientIds []string, now time.Time) (*model.FileUploadResponse, *model.AppError) {
//	files := make([]io.ReadCloser, len(fileHeaders))
//	filenames := make([]string, len(fileHeaders))
//
//	for i, fileHeader := range fileHeaders {
...
//	}
//
//	return app.UploadFiles(teamId, pageId, userId, files, filenames, clientIds, now)
//}
//
//// Uploads some files to the given team and channel as the given user. files and filenames should have
//// the same length. clientIds should either not be provided or have the same length as files and filenames.
//// The provided files should be closed by the caller so that they are not leaked.
//func (app *App) UploadFiles(teamId string, pageId string, userId string, files []io.ReadCloser, filenames []string, clientIds []string, now time.Time) (*model.FileUploadResponse, *model.AppError) {
//	if len(*app.Config().FileSettings.DriverName) == 0 {
...
//		data := buf.Bytes()
//
//		info, data, err := app.DoUploadFileExpectModification(now, teamId, pageId, userId, filenames[i], data)
//		if err != nil {
//			return nil, err
//		}
//
...
//}
//
//func (app *App) DoUploadFile(now time.Time, rawTeamId string, rawPageId string, rawUserId string, rawFilename string, data []byte) (*model.FileInfo, *model.AppError) {
//	info, _, err := app.DoUploadFileExpectModification(now, rawTeamId, rawPageId, rawUserId, rawFilename, data)
//	return info, err
//}
//
//func (app *App) DoUploadFileExpectModification(now time.Time, rawTeamId string, rawPageId string, rawUserId string, rawFilename string, data []byte) (*model.FileInfo, []byte, *model.AppError) {
//	filename := filepath.Base(rawFilename)
//	teamId := filepath.Base(rawTeamId)
//	pageId := filepath.Base(rawPageId)
//	userId := filepath.Base(rawUserId)
//
...
//	info.CreateAt = now.UnixNano() / int64(time.Millisecond)
//
//	pathPrefix := now.Format("20060102") + "/teams/" + teamId + "/pages/" + pageId + "/users/" + userId + "/" + info.Id + "/"
//	info.Path = pathPrefix + filename
//
//	if info.IsImage() {
//		// Check dimensions before loading the whole thing into memory later on
...
//	//}
//
//	if _, err := app.WriteFile(bytes.NewReader(data), info.Path); err != nil {
//		return nil, data, err
//	}
//
//	if result := <-app.Srv.Store.FileInfo().Save(info); result.Err != nil {
...
//}
//
//func (app *App) HandleImages(previewPathList []string, thumbnailPathList []string, fileData [][]byte) {
//	wg := new(sync.WaitGroup)
//
//	for i := range fileData {
//		img, width, height := prepareImage(fileData[i])
...
//			go func(img *image.Image, path string, width int, height int) {
//				defer wg.Done()
//				app.generateThumbnailImage(*img, path, width, height)
//			}(img, thumbnailPathList[i], width, height)
//
//			go func(img *image.Image, path string, width int) {
//				defer wg.Done()
//				app.generatePreviewImage(*img, path, width)
//			}(img, previewPathList[i], width)
//		}
//	}
//	wg.Wait()
...
//}
//
//func (app *App) generateThumbnailImage(img image.Image, thumbnailPath string, width int, height int) {
//	thumbWidth := float64(IMAGE_THUMBNAIL_PIXEL_WIDTH)
//	thumbHeight := float64(IMAGE_THUMBNAIL_PIXEL_HEIGHT)
//	imgWidth := float64(width)
//	imgHeight := float64(height)
...
//	}
//
//	if _, err := app.WriteFile(buf, thumbnailPath); err != nil {
//		mlog.Error(fmt.Sprintf("Unable to upload thumbnail path=%v err=%v", thumbnailPath, err))
//		return
//	}
//}
//
//func (app *App) generatePreviewImage(img image.Image, previewPath string, width int) {
//	var preview image.Image
//
//	if width > IMAGE_PREVIEW_PIXEL_WIDTH {
//		preview = imaging.Resize(img, IMAGE_PREVIEW_PIXEL_WIDTH, 0, imaging.Lanczos)
...
//	}
//
//	if _, err := app.WriteFile(buf, previewPath); err != nil {
//		mlog.Error(fmt.Sprintf("Unable to upload preview err=%v", err), mlog.String("path", previewPath))
//		return
//	}
//}
//
//func (app *App) GetFileInfo(fileId string) (*model.FileInfo, *model.AppError) {
...
//}
//
//func (app *App) CopyFileInfos(userId string, fileIds []string) ([]string, *model.AppError) {
//	var newFileIds []string
//
//	now := model.GetMillis()
//
...
//		fileInfo.PageId = ""
//
//		if result := <-app.Srv.Store.FileInfo().Save(fileInfo); result.Err != nil {
