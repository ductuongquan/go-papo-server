...
import (
	...
	"bitbucket.org/enesyteam/papo-server/services/mfa"
...
func (app *App) CheckPasswordAndAllCriteria(user *model.User, password string, mfaToken string) *model.AppError {
	if err := app.CheckUserPreflightAuthenticationCriteria(user, mfaToken); err != nil {
		return err
	}

	if err := app.checkUserPassword(user, password); err != nil {
		return err
	}

	if err := app.CheckUserPostflightAuthenticationCriteria(user); err != nil {
		return err
	}
...
func (app *App) doubleCheckPassword(user *model.User, password string) *model.AppError {
	if err := checkUserLoginAttempts(user, *app.Config().ServiceSettings.MaximumLoginAttempts); err != nil {
		return err
	}
...
func (app *App) checkUserPassword(user *model.User, password string) *model.AppError {
...
func (app *App) checkLdapUserPasswordAndAllCriteria(ldapId *string, password string, mfaToken string) (*model.User, *model.AppError) {
	if app.Ldap == nil || ldapId == nil {
		err := model.NewAppError("doLdapAuthentication", "api.user.login_ldap.not_available.app_error", nil, "", http.StatusNotImplemented)
		return nil, err
	}

	ldapUser, err := app.Ldap.DoLogin(*ldapId, password)
	if err != nil {
		err.StatusCode = http.StatusUnauthorized
		return nil, err
	}

	if err := app.CheckUserMfa(ldapUser, mfaToken); err != nil {
		return nil, err
	}
...
func (app *App) CheckUserAllAuthenticationCriteria(user *model.User, mfaToken string) *model.AppError {
	if err := app.CheckUserPreflightAuthenticationCriteria(user, mfaToken); err != nil {
		return err
	}
...
func (app *App) CheckUserPreflightAuthenticationCriteria(user *model.User, mfaToken string) *model.AppError {
	if err := app.CheckUserMfa(user, mfaToken); err != nil {
		return err
	}
...
func (app *App) CheckUserPostflightAuthenticationCriteria(user *model.User) *model.AppError {
	...
	//}
	fmt.Println("temporary I can not fix this problem, but it relate to User Post and we don't care now")
	return nil
}
...
func (app *App) authenticateUser(user *model.User, password, mfaToken string) (*model.User, *model.AppError) {
	license := app.License()
	ldapAvailable := *app.Config().LdapSettings.Enable && app.Ldap != nil && license != nil && *license.Features.LDAP
	...
	if user.AuthService == model.USER_AUTH_SERVICE_LDAP {
		...
		ldapUser, err := app.checkLdapUserPasswordAndAllCriteria(user.AuthData, password, mfaToken)
		if err != nil {
			err.StatusCode = http.StatusUnauthorized
			return user, err
		}
