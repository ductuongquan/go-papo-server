...
import (
	...
	"bitbucket.org/enesyteam/papo-server/services/cache2"
	"bitbucket.org/enesyteam/papo-server/utils/imgutils"
	//"bitbucket.org/enesyteam/papo-server/utils/markdown"
	"bytes"
...
func (a *App) getLinkMetadata(requestURL string, timestamp int64, isNewPost bool) (*opengraph.OpenGraph, *model.PostImage, error) {
	...
	if (request.URL.Scheme+"://"+request.URL.Host) == a.GetSiteURL() && request.URL.Path == "/api/v1/image" {
