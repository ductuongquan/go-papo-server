// Copyright (c) 2017-present Ladifire, Inc. All Rights Reserved.
...
func (a *App) GetAnalytics(name string, pageId string) (model.AnalyticsRows, *model.AppError) {
	...
	fmt.Println("skipIntensiveQueries", skipIntensiveQueries)

	//if name == "conversation_counts_day" {
	//	if skipIntensiveQueries {
	//		rows := model.AnalyticsRows{&model.AnalyticsRow{Name: "", Value: -1}}
	//		return rows, nil
	//	}
	//
	//	r := <-a.Srv.Store.FacebookConversation().AnalyticsConversationCountsByDay(pageId)
	//	if r.Err != nil {
	//		return nil, r.Err
	//	}
